...
class Laspatule::Repositories::DB
  class Ingredients
    ...
    def initialize(@db : Laspatule::Repositories::DB)
    end
    ...
    def create(ingredient : Models::CreateIngredient) : Int32
      @db.transaction do |tx|
        cnn = tx.connection
        begin
          er = cnn.exec("INSERT INTO ingredients (name) VALUES (?)", ingredient.name)
          return er.last_inserted_id.to_i
    ...
    def get_by_id(id : Int32) : Models::Ingredient
      @db.transaction do |tx|
        cnn = tx.connection
        cnn.query_one("SELECT id, name FROM ingredients WHERE id = ?", id) do |rs|
